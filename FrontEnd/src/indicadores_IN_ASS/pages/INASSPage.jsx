...
import { getUnidadesInAss, getIndicadoresInAss, generarIN_Ass, getSesionInAss } from '../api/in_ass';
import { UploadIcon, CheckIcon, XIcon, FileIcon } from '../../shared/components/Icons';
import ModalLoading from '../../shared/components/modal/ModalCargando';
import ModalUnidadTardia from '../components/modalUnidadTardia/ModalUnidadTardia';
import INASSErrorToast from './INASSErrorToast';
import INASSValidacionPanel from './INASSValidacionPanel';
import './inass.css';

const INASSPage = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { puedeGenINASS } = useRol();
  ...
  const cargarSesion = useCallback(async () => {
    const s = await getSesionInAss(anio, mes);
    setSesion(s);
  }, [anio, mes]);
  ...
  return (
    <>
        ...
        <main className="ia-main">
          ...
          <div className="ia-step-card ia-step-card--green">
            <div className="ia-step-head">
              ...
              <div>
                <p className="ia-step-title">Datos por Unidad — IASS 01–06</p>
            ...
            >
              <span className="ia-global-badge">IASS 01</span>
      ...
      <INASSValidacionPanel errores={errorLista} trigger={errorListaTrigger} />

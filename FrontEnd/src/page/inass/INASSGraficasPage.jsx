...
import { getInAssDatosGrafica, descargarInAssGuardado, infoBasicaInAass } from '../../api/In_Ass';
import {
...
const INDICADORES = ['IN_ASS 01','IN_ASS 02','IN_ASS 03','IN_ASS 04','IN_ASS 05','IN_ASS 06'];
...
const INASSGraficasPage = () => {
  ...
  const [indSel, setIndSel]           = useState('IN_ASS 02');
  const [cargando, setCargando]       = useState(false);
  const [descargando, setDescargando] = useState(false);
  const [infoAllInAass, setInfoInAss] = useState({});
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        setDatos(null);
        setUnidadSel('');
        const d = await getInAssDatosGrafica(anio);
        setDatos(d);
        ...
        setInfoInAss(respuestaInfo.data);
      } catch (error) {
        console.error('Error cargando datos IN_ASS:', error);
      } finally {
        setCargando(false);
      }
    };
  ...
  // IN_ASS 01 tiene umbrales distintos por tipo de unidad (HGS vs Otros)
  ...
  // rangosSem: umbral principal; rangosSemExtra: segundo umbral (solo IN_ASS 01 en vista mes/acumulado)
  let rangosSem      = null;
                ...
                {unidadesStatus
                  ...
                  .map(({ unidad, color }) => (
                    ...
                    >
                      ...
                      {indSel === 'IN_ASS 01' && hgsSet.has(unidad) && (
...
export default INASSGraficasPage;

"""
Módulo  : ftp_extraer.py
Carpeta : indicadores_FTP/services/
Qué hace: Descarga archivos Excel del FTP y extrae datos por indicador y unidad médica.
Usado en: ftp_extraer_cache.py, FTP_service.py
"""
import io
import json
import pandas as pd
from configs.unidades import (
    UNIDADES_FINALES, UNIDADES_PREVIOS, CLAVE_UNIDADES, NOMBREUNIDADESARCHIVO
)
from configs.settings import RUTA_POBLACION
from indicadores_FTP.services.ftp_conectar import conectar_ftp, desconectar_ftp
...
def procesar_extraccion_ftp(ftp, repo, ano, mes, semana, infoReporte, diccionarioGlobal, logErrores, subcarpeta_base, meses_mapeo=None):
    ...
    for i, unidad_ruta in enumerate(unidades_ruta):
        ...
        try:
            ...
            registrar_error(logErrores, "DESCARGA_FALLIDA", nombre_final, repo,
                            f"{carpeta_remota} · {exc}")
            diccionarioGlobal[nombre_final][repo] = None
...
def ExtraerDatosDelExcel(ruta, infoReporte, mes=None, meses_mapeo=None):
    """
    Retorna (datos, id_error, detalle_error).
    - datos       : lista de valores o None
    - id_error    : clave de logErrores o None si todo OK
    - detalle_error: descripción específica del error (va al campo 'ruta' del log)
    """
        ...
        for letra in letras_columnas:
            if letra == "MESES_CIP01":
                if meses_mapeo and mes is not None:
                    ...
                    if not letra_real:
                        # Mes actual (mes=N) no tiene columna definida en MESES_CIP01

...
from configs.unidades import UNIDADES_HGS_IASS01, ORDEN_IASS01
from indicadores_IASS.services.info_service import obtener_config_indicador
...
def _validar_encabezado_excel(excel_bytes: bytes, unidad_esperada: str, config_numerador: dict) -> list[str]:
    ...
    # Validar que el Excel pertenece a la unidad esperada usando columna E.
    # Solo compara tipo + número (ej. "HGZMF 2") para tolerar diferencias en el nombre de ciudad.
...
def _get_denominador_IASS01(excel_bytes: bytes) -> dict:
    config     = obtener_config_indicador("IASS 01")
    ...
    if not errores:
        ...
        if not any(col_b.str.contains(rf'\b{num}\b', na=False).any() for num in numeros_esperados):
            errores.append("[Denominador global] No se encontraron las unidades de IASS 01 en la columna de unidades.")
...
def calcular_unidad_tardia(
    ...
    (útil cuando solo se quiere actualizar el denominador).
    ...
    for ind in indicadores_seleccionados:
        if ind == "IASS 01":
            if unidad not in ORDEN_IASS01:
                continue
            num = (
                _get_numerador(nums_unidad, "IASS 01").get(unidad, 0)
                if nums_unidad is not None
                else (datos_sesion.get("IASS 01", {}).get(unidad) or {}).get("numerador")
            )
            den = (datos_sesion.get("IASS 01", {}).get(unidad) or {}).get("denominador")
            raw = {unidad: {"numerador": num, "denominador": den}}
            resultado["IASS 01"] = _semaforo_IASS01(raw)
        else:
            num = (

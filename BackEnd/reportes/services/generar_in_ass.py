"""
Módulo  : generar_in_ass.py
Carpeta : reportes/services/
Qué hace: Genera el Excel completo IN_ASS (01-06) con históricos, semáforo y acumulados.
Usado en: indicadores_IN_ASS/services/procesar_service.py, reportes/controllers/reportes_controller.py
"""
import json
import io
import xlsxwriter
import openpyxl  # sigue usándose para guardar el xlsx resultante
from configs.unidades import MESES, ORDEN_IN_ASS01, ORDEN_DEMAS_IN_ASS, UNIDADES_HGS_IN_ASS01
from configs.settings import RUTA_IN_ASS_JSON, RUTA_DATA_IN_ASS

with open(RUTA_IN_ASS_JSON, encoding="utf-8") as _f:
    _CFG = json.load(_f)

RUTA_BASE_IN_ASS = RUTA_DATA_IN_ASS

UNIDADES_IASS     = ORDEN_IN_ASS01 + ["DELEGACION"]
UNIDADES_UCI      = ORDEN_DEMAS_IN_ASS
_UNIDADES_HGS_SET = set(UNIDADES_HGS_IN_ASS01)


def _color_tasa_01(tasa, unidad):
    sem      = _CFG["IN_ASS 01"]["Semaforo"]
...
def _leer_historicos_inass(anio: str, mes_num: int) -> dict:
    """
    ...
    """
    ruta_sesion = RUTA_DATA_IN_ASS / str(anio)
    historicos  = {}

    for ind_n in range(1, 7):
        ind_key   = f"IN_ASS 0{ind_n}"
        ruta_json = ruta_sesion / f"IN_ASS_0{ind_n}.json"
        ...
        try:
            ...
            print(f"[IN_ASS JSON] Error leyendo {ruta_json}: {e}")
            historicos[ind_key] = {}
            continue
    ...
    print(f"[IN_ASS JSON] {total} meses de datos cargados")
    return historicos
...
def _escribir_In_Ass01(libro, estilos):
    fila_titulo1       = 0
    fila_titulo2       = 1
    fila_titulo3       = 2
    fila_nota          = 3
    ...
    hoja = libro.add_worksheet("IN_ASS 01")
...
def Excel_In_Ass01():
    ...
    _escribir_In_Ass01(libro, estilos)
    libro.close()
    salida.seek(0)
    return salida
...
_IASS_UCI_CONFIG = {
    num: {
        "codigo":   _CFG[f"IN_ASS {num}"]["Titulo2"],
        "titulo":   _CFG[f"IN_ASS {num}"]["Titulo1"],
        "hoja":     f"IN_ASS {num}",
        "sub_col1": _CFG[f"IN_ASS {num}"]["subT1"],
        "sub_col2": _CFG[f"IN_ASS {num}"]["subT2"],
...
def _escribir_In_Ass_UCI(libro, estilos, numero: str):
...
def _Excel_In_Ass_UCI(numero: str) -> io.BytesIO:
    salida  = io.BytesIO()
    libro   = xlsxwriter.Workbook(salida)
    estilos = Estilos_IASS01(libro)
    _escribir_In_Ass_UCI(libro, estilos, numero)
    libro.close()
    salida.seek(0)
    return salida


def Excel_In_Ass02(): return _Excel_In_Ass_UCI("02")
def Excel_In_Ass03(): return _Excel_In_Ass_UCI("03")
def Excel_In_Ass04(): return _Excel_In_Ass_UCI("04")
def Excel_In_Ass05(): return _Excel_In_Ass_UCI("05")
def Excel_In_Ass06(): return _Excel_In_Ass_UCI("06")
...
def _escribir_acumulado_inass01(hoja, estilos, all_months):
    N = len(UNIDADES_IASS)
...
def _escribir_acumulado_inass_uci(hoja, estilos, all_months, indicador):
    N         = len(UNIDADES_UCI)
    fila_ini  = N + 8
    fila_del  = 2 * N + 8
...
def _escribir_datos_inass_uci(hoja, estilos, mes_num, datos):
...
def _escribir_datos_inass01(hoja, estilos, mes_num, datos):
    N = len(UNIDADES_IASS)
    if mes_num <= 6:
        fila_inicio = 6
...
def Excel_InAss_Completo(anio: str, mes: str, datos: dict) -> io.BytesIO:
    mes_num      = int(mes)
    ruta_archivo = RUTA_BASE_IN_ASS / str(anio) / f"IN_ASS_{anio}.xlsx"
    historicos   = _leer_historicos_inass(anio, mes_num)
    ...
    _escribir_In_Ass01(libro, estilos)
    for num in ("02", "03", "04", "05", "06"):
        _escribir_In_Ass_UCI(libro, estilos, num)

    hoja01 = libro.get_worksheet_by_name("IN_ASS 01")
    if hoja01:
        all_months_01 = dict(historicos.get("IN_ASS 01", {}))
        for m, mes_datos in all_months_01.items():
            _escribir_datos_inass01(hoja01, estilos, m, mes_datos)
        if datos.get("IN_ASS 01"):
            all_months_01[mes_num] = datos["IN_ASS 01"]
            _escribir_datos_inass01(hoja01, estilos, mes_num, datos["IN_ASS 01"])
        _escribir_acumulado_inass01(hoja01, estilos, all_months_01)
    ...
    for num in ("02", "03", "04", "05", "06"):
        ...
        for m, mes_datos in all_months.items():
            _escribir_datos_inass_uci(hoja, estilos, m, mes_datos)

        _escribir_acumulado_inass_uci(hoja, estilos, all_months, key)
    ...
    try:
        with open(ruta_archivo, "wb") as f:
            f.write(salida.read())
        salida.seek(0)
        print(f"[IN_ASS] Guardado: {ruta_archivo}")
    except PermissionError:
        salida.seek(0)
        raise PermissionError(f"El archivo IN_ASS_{anio}.xlsx está abierto en Excel. Ciérralo e intenta de nuevo.")
